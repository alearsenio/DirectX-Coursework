...
float4 main(InputType input) : SV_TARGET
{
    ...
    //noise calculation
    float noiseScale = 20;
    float noiseIntensity = 10;
    input.position3D.y += time;
    input.position3D.z += time;
    //strech the z component of the position and apply a noise, to recreate sea ridges
    input.position3D.z *= 100 + snoise(input.position3D)/20;	
    //change the pixel brightness
    lightIntensity += snoise(input.position3D/ noiseScale) / noiseIntensity;
    ...
    blendTextureColor = EnviromentalMapColor;
    color *=  blendTextureColor;
	color.a = 0.8f;
	return color;
