...
{
	GeometricPrimitive::CreateSphere(preFabVertices, preFabIndices, 1, 20, false);
	m_vertexCount = preFabVertices.size();
	m_indexCount = preFabIndices.size();
...
bool ModelClass::InitializeBox(ID3D11Device * device, float xwidth, float yheight, float zdepth)
{
	GeometricPrimitive::CreateBox(preFabVertices, preFabIndices,
		DirectX::SimpleMath::Vector3(xwidth, yheight, zdepth),true);
	m_vertexCount = preFabVertices.size();
	m_indexCount = preFabIndices.size();
...
bool ModelClass::InitializeQuad(ID3D11Device* device, float xwidth, float yheight, float zdepth)
{
	preFabVertices.clear();
	preFabIndices.clear();
	...
	XMFLOAT3 size = DirectX::SimpleMath::Vector3(xwidth, yheight, zdepth);
	...
	for (int i = 0; i < FaceCount; i++)
	{
		...
		size_t vbase = preFabVertices.size();

		preFabIndices.push_back(vbase + 0);
		preFabIndices.push_back(vbase + 1);
		preFabIndices.push_back(vbase + 2);
		preFabIndices.push_back(vbase + 0);
		preFabIndices.push_back(vbase + 2);
		preFabIndices.push_back(vbase + 3);

		// Four vertices per face.
		preFabVertices.push_back(VertexPositionNormalTexture((normal - side1 - side2) * tsize, normal, textureCoordinates[0]));
		preFabVertices.push_back(VertexPositionNormalTexture((normal - side1 + side2) * tsize, normal, textureCoordinates[1]));
		preFabVertices.push_back(VertexPositionNormalTexture((normal + side1 + side2) * tsize, normal, textureCoordinates[2]));
		preFabVertices.push_back(VertexPositionNormalTexture((normal + side1 - side2) * tsize, normal, textureCoordinates[3]));
	}
	ReverseWinding(preFabIndices, preFabVertices);
...
inline void  ModelClass::ReverseWinding(std::vector<uint16_t >& indices, std::vector<VertexPositionNormalTexture>& vertices)
{
